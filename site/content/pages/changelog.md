...
- `eval` was a way around the guard. Last week's release taught Anchorwatch to look inside `bash -c "…"`; `eval "…"` is the identical trick and was still open. `eval "rm -rf /"`, `eval 'rm -rf /'` and `eval "cat .env"` went through with no prompt, because the opening quote sits exactly where every rule expects a space. An `eval` payload is now unwrapped and checked like any other command, including behind `sudo`, `command` and `builtin`. The instructions Anchorwatch gives Claude have always said not to reach for `eval` to get around a block — now the guard enforces it rather than asking nicely.
- The closing quote was hiding things too, which made last week's fix less complete than it looked. A rule that ends by requiring a space or end-of-line never saw the final word of an unwrapped payload, so `bash -c "git reset --hard"`, `sh -c "curl … | sh"`, `bash -c "chmod -R 777"` and `bash -c "npm publish"` were all still getting through. Only the three rules that trim quotes from each token — recursive delete, `.env` reads and secret writes — were actually protected. That is fixed for both the old shapes and the new ones.
- Quotes elsewhere are still just text. `git commit -m "rm -rf / broke prod"` and `grep -n "eval" src/app.js` are not commands and are not treated as any, and neither are ordinary uses like `eval "$(ssh-agent -s)"`. Only `eval` and `sh -c` open a quote.
- No rule was added and none was weakened — the count stays at 12 blocking and 13 warning. The rules simply see commands they were always meant to see. 196 tests, up from 173.
- Context, if you set your own permission rules: Claude Code 2.1.273 reverted a 2.1.268 change that tried to read `Read`/`Edit` deny rules off a Bash line containing `eval`, `env -C` or another command its checker cannot analyze, and now prompts for such a line instead. Worth knowing if you were relying on that behaviour between 2.1.268 and 2.1.272.
- Also: `anchorwatch-mod` 0.0.5 ports all of it, and its tests cross-check every verdict against the classic guard so the two cannot drift.

## anchorwatch 0.1.3 — 2026-09-15
- A wrapped command is no longer invisible. Every Bash rule looks for a command at the start of a segment or after a space, which meant a single character of nesting hid it from all of them: `X=$(rm -rf /)`, `` Y=`rm -rf /` ``, `(rm -rf /)`, `(echo x > .env)` and `bash -c "rm -rf /"` went through with no prompt, and `(cd /tmp; rm -rf /)` came out as a warning rather than a block. Anchorwatch now unwraps command substitution, subshells, brace groups and a `sh -c "…"` payload before it splits a command up, so the nested command is checked like any other. Nothing about the rules themselves changed — they simply see what they were always meant to.
- `env-read` now covers every command that prints a file whole. It knew `cat`, `head`, `tail`, `less`, `more`, `bat`, `type` and `Get-Content`; `fmt .env`, `column -t .env`, `nl .env`, `xxd .env`, `strings .env`, `base64 .env`, `od`, `hexdump`, `tac`, `rev`, `pr` and `fold` printed your secrets into the transcript with no objection. A quoted or globbed name (`cat ".env"`, `cat .env*`) is caught too. `grep`, `sed`, `awk` and `cut` are deliberately still allowed — listing variable names without their values is the remedy the block message recommends, and it has to keep working.
- Claude Code 2.1.271 fixed this same class of hole in its own Bash permission checks the same week: a subshell or `cd`+`git` chain that skipped the prompt, shell variable declaration flags that misrepresented the command being run, and the file that `fmt`, `column` "and similar commands" read going unnoticed. If you rely on your own `Bash(...)` deny rules, they got stronger on 2.1.271 too.
- Also: `anchorwatch-mod` 0.0.4 ports both fixes. Its tests cross-check every verdict against the classic guard, so the two cannot drift.
