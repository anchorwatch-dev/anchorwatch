...
| `env-read` | <span class="tag block">block</span> | any command that prints a file whole — `cat`, `head`, `tail`, `less`, `more`, `bat`, `type`, `Get-Content`, `fmt`, `column`, `nl`, `od`, `xxd`, `hexdump`, `strings`, `base64`, `tac`, `rev`, `pr`, `fold` — on `.env`, `.env.*` or `.env*` (not `.env.example/.sample/.template/.dist`) | `grep -oE '^[A-Za-z_][A-Za-z0-9_]*' .env` to list names; ask the user for a value. `grep`, `sed`, `awk` and `cut` are deliberately not on the list, so the remedy still runs |
