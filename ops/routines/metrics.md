...
5. Append the row to the table in `ops/METRICS.md`, and update the ledger table on `site/content/pages/experiment.md` to match. On that page also update the status strip (next weekly report date; owner's time this week — estimate from the number of `needs-human` items and PRs the owner merged by hand, default "~1 h") and add a dated entry to the `## Log` timeline summarising the week in one or two sentences. Add a 3–6 sentence assessment under "Assessment log": what moved, what didn't, one hypothesis, and the single next experiment. Be plain about failure.
