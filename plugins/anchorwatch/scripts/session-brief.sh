...
n_block=0; n_warn=0
for pair in rm-recursive-dangerous:block rm-recursive:warn git-force-push-protected:block git-force-push:warn git-push-delete:warn git-destructive:block sql-destructive:block pipe-to-shell:block disk-destroy:block perm-broad:block env-read:block secret-write:block env-dump:warn publish:warn sudo:warn kill-broad:warn system-config:warn secret-files:block git-internals:block lockfiles:warn self-config:warn infra-files:warn outside-project:warn secret-read:block secret-scan:warn; do
  r="${pair%%:*}"; d="${pair##*:}"
